...
name: planner
description: |
  Architecture planning specialist. Read-only, produces structured
  implementation plans. Use when the user wants to analyze, design,
  or plan without making code changes.
tools:
  - Read
  - Glob
  - Grep
...
**Frontmatter 字段说明**（来源：[官方 Subagents 文档](https://code.claude.com/docs/en/sub-agents)）：

| 字段 | 必填 | 说明 |
|------|------|------|
| `name` | ✅ | Agent 标识符，用于 `@name` 调用 |
| `description` | ✅ | 用途描述，Claude 用它判断何时委派 |
| `tools` | ❌ | 可用工具列表（如 `Read`, `Glob`, `Grep`, `Edit`） |
| `disallowedTools` | ❌ | 禁止使用的工具 |
| `model` | ❌ | 指定模型（如 `haiku` 可降低成本） |
| `permissionMode` | ❌ | 权限模式：`plan` / `suggest` / `auto` 等 |
| `memory` | ❌ | 记忆范围：`project` / `user` / `none` |
...
| `tools` | `tools` / `disallowedTools` frontmatter |
| `permissions` | `permissionMode` frontmatter + `settings.json` |
| Tab 切换 | `@agent-name` 切换 |
